 ...
 * @FilePath: \day01\7.js-ticket\wechat\wechat.js
 */
...
class Wechat{
    ...
    isValidAccessToken(data){
      console.log(data,'isValidAccessToken')
      //无效参数
     ...
     * @return {*}
     */
    isValidTicket(data,type){
      console.log(data,type,'isValidTicket')
      //无效参数
     ...
     */    
    fetchTicket(){
      if(this.ticket && this.ticket_expires_in && this.isValidTicket(this,'在class上的取值》》》》》》') ){
        return Promise.resolve({
          ticket: this.ticket,
      ...
      return this.readTicket()
        .then(async res=>{
          console.log('票据-- 本地有文件')
          // 判断是否有效
          if(this.isValidTicket(res)){
            console.log('票据-- 本地有文件--有效')
          // 有效
          // resolve(res)
          return Promise.resolve(res)
          }else{
            console.log('票据-- 本地有文件--过期')
            // 过期
